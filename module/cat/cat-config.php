...
die('<a href="http://help.rashcms.com/modules" target=_blank>RashCMS</a> :: Invalid calling of '.basename (__FILE__));
	...
	if(isset($_GET['module']))
	{
		...
		@$From = (!is_numeric($From)) ? 1 : abs($From);
				...
				$From = 0;
				$post_q = "select * FROM `data` WHERE  `date` <= '$ctimestamp' AND (`expire`='0' OR `expire`='' OR `expire`>'$ctimestamp') AND `cat_id`='$_GET[catid]'  order by `id` $type LIMIT $From,$RPP";
				define('customized_post_query_value',$post_q);
				$t_pr = $d->getrows("select `id` FROM `data` WHERE  `date` <= '$ctimestamp' AND (`expire`='0' OR `expire`='' OR `expire`>'$ctimestamp') AND `cat_id`='$_GET[catid]'",true);
				...
				$ctimestamp = time();
				define('customized_post_query',true);
				$post_q = "select * FROM `data` WHERE  `date` <= '$ctimestamp' AND (`expire`='0' OR `expire`='' OR `expire`>$ctimestamp) AND `id`='$_GET[postid]' LIMIT 1";
				define('customized_post_query_value',$post_q);
